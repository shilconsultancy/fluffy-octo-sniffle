            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="quote_date" id="quote_date" value="{{ old('quote_date', $quote->quote_date->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-macgray-300 shadow-sm sm:text-sm">
                    </div>
                    <div>
                        <label for="expiry_date" class="block text-sm font-medium text-macgray-700">Expiry Date</label>
                        <input type="date" name="expiry_date" id="expiry_date" value="{{ old('expiry_date', $quote->expiry_date->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-macgray-300 shadow-sm sm:text-sm">
                    </div>
                </div>
...
<script>
    function quoteForm(config) {
        return {
            items: config.items,
            lines: config.lineItems, // Changed from 'old' to 'lineItems' for clarity
